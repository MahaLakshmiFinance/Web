...
if(isset($_SESSION['id_available'])){
    if($_SESSION['id_available']==True){
        echo '<script>
            ...
            document.forms["delete_user"].style=""
        </script>';
        fetchDetails($_SESSION['id']);
    }

}
if(isset($_SESSION['error_typed_delete'])){
    if($_SESSION['error_typed_delete']==True)
        echo '<script>
...
function fetchDetails($id){
      ...
      while($row = $result->fetch_assoc()){
       ...
       echo "<script>
        setTimeout(function(){
            document.forms['delete_user']['username'].value = '".$row['username']."';
            document.forms['delete_user']['fname'].value = '".$row['first_name']."';
            document.forms['delete_user']['lname'].value = '".$row['last_name']."';
            document.forms['delete_user']['cntact_num'].value = '".$row['contact_num']."';
            document.forms['delete_user']['alt_cntact_num'].value = '".$row['alternate_num']."';
            document.forms['delete_user']['dno'].value = '".$row['d_no']."';
            document.forms['delete_user']['street'].value = '".$row['street']."';
            document.forms['delete_user']['locality'].value = '".$row['locality']."';
            document.forms['delete_user']['location'].value = '".$row['town_or_city']."';
            document.forms['delete_user']['district'].value = '".$row['district']."';
            document.forms['delete_user']['pincode'].value = '".$row['pincode']."';
            document.forms['delete_user']['refer_id'].value = '".$row['refer_username']."';
            document.forms['delete_user']['refer_name'].value = '".$row['refer_name']."';
       },20);
       </script>" ;
       break;
      }
...
$_SESSION['error_typed_delete'] = False;
?>
...
<div class="templatemo-content-widget white-bg">
    ...
    <input type="number" id="x" class="form-control" name="subcontent_num" value="3" style="display:none;" placeholder="Aadhar / Phone Number">
    </div>
    ...
    <form action="php/content/content1/delete_user.php" name="delete_user" style="display:none;"class="templatemo-login-form" method="POST" enctype="multipart/form-data">
